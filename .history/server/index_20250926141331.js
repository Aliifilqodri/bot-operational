...
// Escape Markdown (biar ga error di Telegram)
function escapeMarkdown(text = '') {
  return text.replace(/[_*[\]()~`>#+\-=|{}.!]/g, '\\$&');
}

// Ganti seluruh blok reply lama dengan ini
app.post('/api/tickets/:id/reply', authMiddleware, upload.single('photo'), async (req, res) => {
  try {
    ...
    // Kalau ada file -> kirim foto dengan caption
    if (req.file) {
      await bot.sendPhoto(ticket.chatId, req.file.buffer, {
        caption: `💬 *Update dari Admin untuk tiket ${escapeMarkdown(ticket.ticketCode)}:*\n\n${escapeMarkdown(balasan)}`,
        parse_mode: "MarkdownV2",
        reply_to_message_id: ticket.messageId
      });
    } else {
      // Kalau ga ada file -> kirim teks
      await bot.sendMessage(ticket.chatId, 
        `💬 *Update dari Admin untuk tiket ${escapeMarkdown(ticket.ticketCode)}:*\n\n${escapeMarkdown(balasan)}`, 
        {
          parse_mode: "MarkdownV2",
          reply_to_message_id: ticket.messageId
        }
...
app.get('/api/tickets', async (req, res) => {
  try {
    ...
    const finalStats = allFilteredTickets.reduce((acc, ticket) => {
      if(ticket.status === 'Diproses') acc.totalDiproses++;
      if(ticket.status === 'Selesai') acc.totalSelesai++;
      ...
      if(ticketDate >= todayStart){
        if(ticket.status === 'Diproses') acc.statsToday.diproses++;
        if(ticket.status === 'Selesai') acc.statsToday.selesai++;
      } else if(ticketDate >= yesterdayStart && ticketDate < todayStart){
        if(ticket.status === 'Diproses') acc.statsYesterday.diproses++;
        if(ticket.status === 'Selesai') acc.statsYesterday.selesai++;
      }
...
app.post('/api/tickets/:id/reply', async (req, res) => {
  try {
    ...
    const pesanGabungan = `💬 <b>Update dari IT TA:</b>\n${balasan}`;

    try {
      await bot.sendMessage(ticket.chatId, pesanGabungan, {
        parse_mode: 'HTML',
        reply_to_message_id: ticket.messageId
      });
    } catch(err) {
