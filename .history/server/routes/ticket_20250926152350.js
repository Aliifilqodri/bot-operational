...
const Ticket = require('../models/Ticket'); // Pastikan path model Anda benar
const excel = require('exceljs');
const multer = require('multer');

// Asumsi Anda akan memindahkan authMiddleware dan bot ke file terpisah
// Jika tidak, Anda perlu meneruskannya dari file index.js utama Anda
const { authMiddleware } = require('../middleware/authMiddleware'); // Sesuaikan path
const { bot, escapeMarkdownV2 } = require('../utils/telegramSetup'); // Sesuaikan path
...
// (Anda perlu memindahkan logika get tickets & stats ke sini)
...
// [POST] /api/tickets/:id/reply - Mengirim balasan ke pengguna (DIPERBARUI)
...
// [PATCH] /api/tickets/:id/set-status - Mengubah status tiket (BARU)
router.patch('/:id/set-status', authMiddleware, async (req, res) => {
    try {
        ...
        // Jika statusnya 'Done', catat juga waktu selesainya
        if (status === 'Done') {
            updateData.completedAt = new Date();
        } else {
            // Jika status diubah kembali dari 'Done', hapus waktu selesainya
        ...
        // Kirim notifikasi ke Telegram bahwa status telah berubah
        const statusUpdateMessage = `ℹ️ Status tiket *${escapeMarkdownV2(ticket.ticketCode)}* diubah menjadi *${escapeMarkdownV2(status)}*\\.`;
        
        // Gunakan try-catch untuk pengiriman pesan agar tidak menggagalkan response API
        try {
...
// (Anda perlu memindahkan logika set PIC ke sini)
...
// (Anda perlu memindahkan logika export ke sini)
