...
router.post('/:id/reply', authMiddleware, upload.single('photo'), async (req, res) => {
  try {
    ...
    const replyNotification = `💬 *Update dari Admin untuk tiket ${ticket.ticketId}:*\n\n${balasan}`;
...
router.patch('/:id/set-status', authMiddleware, async (req, res) => {
    try {
        ...
        const statusUpdateMessage = `ℹ️ Status tiket *${escapeMarkdownV2(ticket.ticketCode)}* diubah menjadi *${escapeMarkdownV2(ticket.status)}*\\.`;
        try {
