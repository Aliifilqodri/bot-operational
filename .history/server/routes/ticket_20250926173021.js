...
app.patch('/api/tickets/:id/set-status', async (req, res) => {
  try {
    ...
    // Jika perlu, kirim update ke Telegram
    try {
      await bot.sendMessage(ticket.chatId, `⚙️ Status tiket <code>${ticket.ticketCode}</code> diperbarui menjadi: ${status}`, {
        parse_mode: 'HTML',
        reply_to_message_id: ticket.messageId
      });
    } catch (err) {
      console.error('❌ Gagal mengirim update status ke Telegram:', err);
    }
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        // TODO: statistik bisa ditambahkan di sini jika dibutuhkan
        const stats = {}; 
...
router.patch('/reply/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // on hold & waiting third party masih bisa reply
