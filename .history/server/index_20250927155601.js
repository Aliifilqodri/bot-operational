...
// --- BARU: Fitur Perkenalan Bot di Grup ---
let BOT_INFO = null;
(async () => {
  try {
    BOT_INFO = await bot.getMe();
    console.log(`✅ Info bot diterima: ${BOT_INFO.username} (ID: ${BOT_INFO.id})`);
  } catch (error) {
...
bot.on('new_chat_members', (msg) => {
  if (!BOT_INFO) return; // Jangan lakukan apa-apa jika info bot belum siap

  const chatId = msg.chat.id;
  const newMembers = msg.new_chat_members;

  newMembers.forEach(member => {
    // Cek apakah member baru yang masuk adalah bot ini sendiri
...
Terima kasih sudah menambahkan saya ke grup ini 🙏
      ...
      // Kirim pesan perkenalan dengan tombol
      sendMessage(chatId, introText, {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [
            [{ text: "Dashboard Web", url: "http://localhost:5173/" }], // Ganti dengan URL dashboard Anda
            [{ text: "Hubungi Admin", url: "https://t.me/username_admin" }] // Ganti dengan username admin
          ]
        }
      });
    }
