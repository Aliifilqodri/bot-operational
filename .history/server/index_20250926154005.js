...
const { authMiddleware } = require('./middleware/authMiddleware'); // Pastikan Anda memiliki file ini
  ...
  try {
    ...
    const finalStats = allFilteredTickets.reduce((acc, ticket) => {
      ...
      if(ticketDate >= todayStart){
        if(ticket.status === 'Done') acc.statsToday.done++;
        else acc.statsToday.inProgress++;
      } else if(ticketDate >= yesterdayStart && ticketDate < todayStart){
        if(ticket.status === 'Done') acc.statsYesterday.done++;
        else acc.statsYesterday.inProgress++;
      }
...
// POST balasan (tanpa mengubah status)
app.post('/api/tickets/:id/reply', authMiddleware, upload.single('photo'), async (req, res) => {
  try {
    ...
    // Hanya menyimpan pesan admin, tidak mengubah status
    ...
    await ticket.save();
    res.json({ success: true, message: 'Balasan berhasil terkirim.' });
  } catch(err){
...
// PATCH set Status (ENDPOINT BARU)
app.patch('/api/tickets/:id/set-status', authMiddleware, async (req, res) => {
  try {
    ...
    if (status === 'Done') {
      ...
      // Jika status dikembalikan dari 'Done', hapus tanggal selesai
    ...
    // Kirim notifikasi perubahan status ke Telegram
    ...
    try {
        ...
        console.error("Gagal mengirim notif status ke Telegram:", telegramError.message);
    }
