...
app.patch('/api/tickets/:id/set-status', async (req, res) => {
  try {
    ...
    console.log('🔹 Request set-status:', req.params.id, status);
    ...
    if(ticket.status === 'Done') {
      console.log('❌ Tiket sudah Done, tidak bisa diubah lagi');
      return res.status(400).json({ message: 'Tiket sudah selesai, tidak bisa diubah lagi.' });
    }
    ...
    await ticket.save();
    console.log('✅ Status tiket diperbarui:', ticket);

    res.json(ticket);
  } catch(err){
    console.error('❌ Error set-status:', err);
