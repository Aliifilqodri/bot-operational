...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  // --- PERUBAHAN #1: Batas kata diubah menjadi 5 ---
  const TRUNCATE_WORDS = 5;
  const words = (ticket.text || "").split(' ');
  const isLongText = words.length > TRUNCATE_WORDS;

  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', border: `1px solid ${CONFIG.COLORS.border}`, display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      
      {ticket.photoUrl && (
        <button 
          onClick={() => onImageClick(ticket.photoUrl)} 
          style={{ position: 'absolute', top: '-16px', right: '16px', zIndex: 10, backgroundColor: 'white', border: `1px solid ${CONFIG.COLORS.border}`, borderRadius: '50%', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', transition: 'transform 0.2s ease-in-out' }}
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '20px' }}>
        <div>
          <p style={{ 
              ...
              display: '-webkit-box',
              WebkitLineClamp: 2, // <-- PERUBAHAN #2: Batas baris diubah menjadi 2
              WebkitBoxOrient: 'vertical',
              minHeight: '52px' // Memaksa tinggi blok setara 2 baris (16px * 1.6 * 2)
            }}>
        ...
        <div style={{ padding: '20px', backgroundColor: CONFIG.COLORS.actionBg, borderTop: `1px solid ${CONFIG.COLORS.border}`, borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px', flexShrink: 0 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              <select className="form-select form-select-sm" value={STATUS_LIST.includes(ticket.status) ? ticket.status : 'Diproses'} onChange={handleStatusChange}>{STATUS_LIST.map(status => (<option key={status} value={status}>{status}</option>))}</select>
              <select className="form-select form-select-sm" value={ticket.pic || 'Belum Ditentukan'} onChange={handleSetPic}><option value="Belum Ditentukan">Pilih PIC...</option>{picList.filter(pic => pic !== 'Belum Ditentukan').map(pic => (<option key={pic} value={pic}>{pic.toUpperCase()}</option>))}</select>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button type="button" onClick={handleReply} className="btn btn-dark btn-sm" style={{ whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '6px' }}><Icon path={CONFIG.ICONS.send} color="#fff" size="1em" /> Kirim</button>
            </div>
