...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  // ... (semua fungsi handle seperti handleSetPic, handleStatusChange, handleReply tetap sama)
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setBalasan(''); setSelectedFile(null); refreshData();
    } catch (err) {
  ...
  const TRUNCATE_WORDS = 15; // Kita bisa perpanjang sedikit karena layout lebih fleksibel
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 4px 8px rgba(0,0,0,0.02),0 10px 20px rgba(0,0,0,0.06)', border: `1px solid ${CONFIG.COLORS.border}`, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* ===== BAGIAN 2: KONTEN UTAMA (DESKRIPSI) ===== */}
      ...
      {/* ===== BAGIAN 3: AREA INFO & AKSI (BARU) ===== */}
      <div style={{ padding: '20px', borderTop: `1px solid ${CONFIG.COLORS.border}`, backgroundColor: '#fdfdfd' }}>
        {/* Tombol Aksi (Lampiran & Selengkapnya) */}
        {(ticket.photoUrl || isLongText) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            {ticket.photoUrl && (
              <button onClick={() => onImageClick(ticket.photoUrl)} style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', borderRadius: '8px', backgroundColor: '#f9fafb', border: `1px solid ${CONFIG.COLORS.border}`, color: CONFIG.COLORS.text.label, fontWeight: '500', fontSize: '0.9rem', transition: 'background-color 0.2s' }}>
      ...
      {/* ===== BAGIAN 4: FOOTER / TIMELINE ===== */}
      <div style={{ padding: '12px 20px', borderTop: `1px solid ${CONFIG.COLORS.border}`, backgroundColor: '#fdfdfd', fontSize: '0.85rem' }}>
      ...
      {/* ===== BAGIAN 5: AREA AKSI (REPLY) ===== */}
      {ticket.status.toLowerCase() !== 'done' && (
        <div style={{ padding: '20px', backgroundColor: CONFIG.COLORS.actionBg, borderTop: `1px solid ${CONFIG.COLORS.border}`, borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px' }}>
