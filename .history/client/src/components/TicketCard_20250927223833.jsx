...
const CONFIG = {
  ...
  ICONS: {
    ...
    // --- PERUBAHAN 1: Menambahkan ikon paperclip ---
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 4px 8px rgba(0,0,0,0.02),0 10px 20px rgba(0,0,0,0.06)', border: `1px solid ${CONFIG.COLORS.border}`, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '20px', lineHeight: 1.6, flexGrow: 1 }}>
        <p style={{ margin: '0 0 16px 0', fontSize: '1rem', color: CONFIG.COLORS.text.primary, wordBreak: 'break-word' }}>
          {isLongText ? words.slice(0, TRUNCATE_WORDS).join(' ') + '...' : ticket.text || '(Tidak ada deskripsi)'}
        </p>
        
        {/* --- PERUBAHAN 2: Mengganti gambar dengan tombol lampiran --- */}
        <div style={{ display: 'flex', gap: '12px' }}>
            {ticket.photoUrl && (
              <button 
                onClick={() => onImageClick(ticket.photoUrl)} 
                style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', borderRadius: '8px', backgroundColor: '#f9fafb', border: `1px solid ${CONFIG.COLORS.border}`, color: CONFIG.COLORS.text.label, fontWeight: '500', fontSize: '0.9rem' }}
              >
