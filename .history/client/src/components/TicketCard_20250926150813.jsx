...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  const handleReplyAndComplete = async () => {
    if (!balasan.trim()) {
      return toast.error('Balasan tidak boleh kosong untuk menyelesaikan tiket.');
    }
    ...
    if (selectedFile) {
      formData.append('photo', selectedFile); // key harus sama dengan backend
    }
    ...
    try {
      ...
      toast.success('Tiket berhasil diselesaikan!', { id: toastId });
      setBalasan('');
