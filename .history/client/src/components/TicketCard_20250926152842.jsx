...
// --- KONFIGURASI DESAIN (Diperbarui dengan status baru) ---
const CONFIG = {
  COLORS: {
    // Kunci disamakan dengan STATUS_LIST (lowercase)
    'diproses': { bg: '#fffbeb', text: '#d97706', border: '#fbb_f_1_9' },
...
// --- DAFTAR STATUS BARU ---
const STATUS_LIST = ['pilih status', 'Diproses', 'On Hold', 'Waiting Third Party', 'Done'];
...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  // Normalisasi key status untuk objek warna
  ...
  // Tanggal selesai hanya ditampilkan jika statusnya "Done"
  ...
  return (
    <div
      style={{
        ...
        height: '100%' // Membuat kartu mengisi tinggi grid
      }}
      onMouseEnter={e => {
    ...
    >
      ...
      <div style={{ padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${CONFIG.COLORS.border}`, backgroundColor: statusColors.bg, borderTopLeftRadius: '16px', borderTopRightRadius: '16px' }}>
        <span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '600', backgroundColor: statusColors.text, color: '#ffffff' }}>
          {ticket.status}
        </span>
      ...
      <div style={{ padding: '20px', borderTop: `1px solid ${CONFIG.COLORS.border}`, backgroundColor: '#fdfdfd' }}>
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.9rem', color: CONFIG.COLORS.text.primary }}>
            ...
            {completedDate ? (
              <span style={{ fontWeight: '600', color: CONFIG.COLORS.done.text }}>{completedDate}</span>
            ) : (
        ...
        <div style={{ marginTop: 'auto', padding: '20px', backgroundColor: CONFIG.COLORS.actionBg, borderTop: `1px solid ${CONFIG.COLORS.border}`, borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <select className="form-select form-select-sm" value={ticket.status} onChange={handleStatusChange}>
                {STATUS_LIST.map(status => (<option key={status} value={status}>{status}</option>))}
              </select>
              <select className="form-select form-select-sm" value={ticket.pic || 'BelumDitentukan'} onChange={handleSetPic}>
                {picList.map(pic => (<option key={pic} value={pic}>{pic === 'BelumDitentukan' ? 'Pilih PIC...' : pic.toUpperCase()}</option>))}
              </select>
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button type="button" onClick={handleReply} className="btn btn-dark btn-sm" style={{ whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Icon path={CONFIG.ICONS.send} color="#fff" size="1em" />
                Kirim
              </button>
            </div>
