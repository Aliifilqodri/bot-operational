    ...
    <>
      ...
      <div style={{ backgroundColor: '#f8f9fa' }}>
        <div className="container py-4">
          ...
          {/* --- PENAMBAHAN: KARTU STATISTIK UTAMA --- */}
            ...
            <div className="col-lg-3 col-md-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body d-flex align-items-center">
                  ...
                  <div>
                    <h2 className="mb-0 fw-bold">{stats.totalDiproses || 0}</h2>
                    <p className="mb-0 text-muted">Diproses</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="col-lg-3 col-md-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body d-flex align-items-center">
                  ...
                  <div>
                    <h2 className="mb-0 fw-bold">{stats.totalSelesai || 0}</h2>
          ...
          {/* --- PENYEMPURNAAN: CHART DIBUNGKUS CARD --- */}
          <div className="row g-4 mb-4">
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Statistik Pengerjaan</div>
                <div className="card-body"><StatusChart data={{ totalDiproses: stats.totalDiproses, totalSelesai: stats.totalSelesai }} /></div>
              </div>
            </div>
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Distribusi per PIC</div>
                <div className="card-body"><PicChart data={stats.picData} /></div>
              </div>
            </div>
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Kategori Case</div>
                <div className="card-body">{stats.caseData && Object.keys(stats.caseData).length > 0 ? <CaseChart data={stats.caseData}/> : <div className="alert alert-info text-center m-0">Tidak ada data.</div>}</div>
              </div>
            </div>
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Performa Harian</div>
                <div className="card-body"><DailyChart today={stats.statsToday} yesterday={stats.statsYesterday} /></div>
              </div>
            </div>
          </div>

          {/* --- PENYEMPURNAAN: FILTER DAN TIKET DALAM SATU CARD --- */}
            ...
            <div className="card-body p-4">
              <div className="d-flex justify-content-center border-bottom mb-4 pb-3">
                <ul className="nav nav-pills nav-fill gap-2 p-1 bg-light rounded-pill">
                  ...
                  <li className="nav-item"><button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'processed' ? 'active' : ''}`} onClick={() => handleViewChange('processed')}><FaSyncAlt /> Proses <span className="badge rounded-pill bg-warning text-dark">{(stats.totalDiproses || 0) + (stats.totalMenungguApproval || 0)}</span></button></li>
                  <li className="nav-item"><button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'completed' ? 'active' : ''}`} onClick={() => handleViewChange('completed')}><FaCheckCircle /> Selesai <span className="badge rounded-pill bg-success">{stats.totalSelesai}</span></button></li>
                </ul>
              </div>
              ...
              {/* --- PENYEMPURNAAN: GRID TIKET YANG RAPI SEMPURNA --- */}
