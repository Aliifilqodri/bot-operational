...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  return (
    // KUNCI #1: Tambahkan 'position: relative' agar bisa menampung penjepit kertas
    <div style={{ backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', border: `1px solid ${CONFIG.COLORS.border}`, display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      {/* KUNCI #2: Ini adalah ikon 'penjepit kertas' yang menggantung */}
      {ticket.photoUrl && (
        <button 
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '20px' }}>
        <div>
          <p style={{ margin: 0, fontSize: '1rem', color: CONFIG.COLORS.text.primary, wordBreak: 'break-word', lineHeight: 1.6, minHeight: '77px', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
          ...
          <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px', minHeight: '36px' }}>
            {/* Tombol 'Lihat Lampiran' sudah tidak ada di sini lagi */}
            {isLongText && (
