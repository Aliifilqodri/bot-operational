            ...
            <div className="card-body p-4">
              ...
              <div className="d-flex justify-content-center border-bottom mb-4 pb-3">
                <ul className="nav nav-pills nav-fill gap-2 p-1 bg-light rounded-pill">
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'all' ? 'active' : ''}`} onClick={() => handleViewChange('all')}>
                      <FaTicketAlt /> Semua Tiket <span className="badge rounded-pill bg-secondary">{totalTickets}</span>
                    </button>
                  </li>
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'processed' ? 'active' : ''}`} onClick={() => handleViewChange('processed')}>
                      <FaSyncAlt /> Proses <span className="badge rounded-pill bg-warning text-dark">{(stats.totalDiproses || 0) + (stats.totalMenungguApproval || 0)}</span>
                    </button>
                  </li>
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'completed' ? 'active' : ''}`} onClick={() => handleViewChange('completed')}>
                      <FaCheckCircle /> Selesai <span className="badge rounded-pill bg-success">{stats.totalSelesai}</span>
                    </button>
                  </li>
                </ul>
              </div>
              ...
              <div className="row g-3 align-items-end mb-4">
                <div className="col-lg-4 col-md-12">
                  <label className="form-label">Cari Tiket</label>
                ...
                <div className="col-lg-2 col-md-4">
                  ...
                  <select className="form-select" value={picFilter} onChange={e => { setPicFilter(e.target.value); setCurrentPage(1); }}>
                    ...
                    {stats.picList && stats.picList.map(pic => <option key={pic} value={pic}>{pic === 'BelumDitentukan' ? 'Belum Ada' : pic.toUpperCase()}</option>)}
                  </select>
                </div>
                <div className="col-lg-2 col-md-4">
                ...
                <div className="col-lg-2 col-md-12 d-flex gap-2">
                  ...
                  <button className="btn btn-success w-100" onClick={handleExport}><FaFileExcel className="me-1"/> Ekspor</button>
                </div>
              </div>
