...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  return (
    <div style={{ 
      ...
      height: '100%',
      // KUNCI #1: Tambahkan position: 'relative' pada pembungkus utama
      position: 'relative' 
    }}>
      
      {/* KUNCI #2: Tambahkan tombol lampiran yang 'menggantung' di sini */}
      {ticket.photoUrl && (
        <button 
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '20px' }}>
        <div>
          ...
          <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px', minHeight: '36px' }}>
            {/* KUNCI #3: Tombol 'Lihat Lampiran' yang lama sudah dihapus dari sini */}
            {isLongText && (
