...
import { 
  ...
  FaTasks, FaSpinner, FaCheckDouble, FaHourglassHalf 
} from 'react-icons/fa';
...
function DashboardPage() {
  ...
  const totalTickets = stats.totalDiproses + stats.totalSelesai;
      ...
      <div style={{ backgroundColor: '#f8f9fa' }}>
        <div className="container py-4">
          ...
          <div className="d-flex justify-content-between align-items-center mb-4 p-3 bg-white rounded-3 shadow-sm">
            <div>
              <h1 className="h3 mb-0">📊 Dashboard Kendala Operasional</h1>
            ...
            <div className="col-lg-3 col-md-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body d-flex align-items-center">
                  ...
                  <div>
                    <h2 className="mb-0 fw-bold">{stats.totalDiproses}</h2>
                    <p className="mb-0 text-muted">Tiket Diproses</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="row g-4 mb-4">
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Statistik Pengerjaan Kendala</div>
                <div className="card-body"><StatusChart data={{ totalDiproses: stats.totalDiproses, totalSelesai: stats.totalSelesai }} /></div>
              </div>
            </div>
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Distribusi Tiket per PIC</div>
                <div className="card-body"><PicChart data={stats.picData} /></div>
              </div>
            </div>
            <div className="col-lg-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-header bg-white fw-bold">Kategori Case Terbanyak</div>
                <div className="card-body">
                  {stats.caseData && Object.keys(stats.caseData).length > 0 ? 
                    <CaseChart data={stats.caseData}/> :
                    <div className="alert alert-info text-center m-0">Tidak ada data case.</div>
                  }
                </div>
              </div>
            </div>
            ...
            <div className="card-body">
                ...
                <ul className="nav nav-pills nav-fill gap-2 p-1 bg-light rounded-pill">
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center justify-content-center gap-2 ${activeView === 'all' ? 'active shadow-sm' : ''}`} onClick={() => handleViewChange('all')}>
                      <FaTicketAlt /> Semua Tiket <span className="badge rounded-pill bg-secondary">{totalTickets}</span>
                    </button>
                  </li>
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center justify-content-center gap-2 ${activeView === 'processed' ? 'active shadow-sm' : ''}`} onClick={() => handleViewChange('processed')}>
                      <FaSyncAlt /> Tiket Diproses <span className="badge rounded-pill bg-warning text-dark">{stats.totalDiproses}</span>
                    </button>
                  </li>
                  <li className="nav-item">
                    <button className={`nav-link d-flex align-items-center justify-content-center gap-2 ${activeView === 'completed' ? 'active shadow-sm' : ''}`} onClick={() => handleViewChange('completed')}>
                      <FaCheckCircle /> Tiket Selesai <span className="badge rounded-pill bg-success">{stats.totalSelesai}</span>
                    </button>
                  </li>
                </ul>
              </div>
                ...
                <div className="card-body">
                    <div className="row g-3 align-items-end">
                      <div className="col-lg-4 col-md-12">
                          <label className="form-label">Cari Tiket (Kode, Deskripsi, User)</label>
                      ...
                      <div className="col-lg-2 col-md-12 d-flex gap-2 justify-content-end">
                          <button className="btn btn-secondary w-100" onClick={handleResetFilters}>Reset</button>
                          <button className="btn btn-success w-100 d-flex align-items-center justify-content-center gap-1" onClick={handleExport}><FaFileExcel/> Ekspor</button>
                      </div>
                    </div>
                </div>
              </div>
              ...
              <div className="row g-4">
                {tickets.length > 0 ? tickets.map(ticket => (
                  <div key={ticket._id} className="col-12 col-md-6 col-lg-4 d-flex">
