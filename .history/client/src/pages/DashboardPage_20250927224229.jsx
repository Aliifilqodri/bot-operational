  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        page: currentPage,
        limit: 9, // Diubah menjadi 9 agar pas 3 per baris
        status: statusForApi,
        pic: picFilter,
        startDate,
        endDate,
      ...
      <div style={{ backgroundColor: '#f8f9fa' }}>
        <div className="container py-4">
          ...
          <div className="row g-4 mb-4">
            ...
            <div className="col-lg-6"><div className="card border-0 shadow-sm h-100"><div className="card-header bg-white fw-bold">Kategori Case</div><div className="card-body">{stats.caseData && Object.keys(stats.caseData).length > 0 ? <CaseChart data={stats.caseData}/> : <div className="alert alert-info text-center m-0">Tidak ada data.</div>}</div></div></div>
            ...
            <div className="card-body p-4">
              <div className="d-flex justify-content-center border-bottom mb-4 pb-3">
                <ul className="nav nav-pills nav-fill gap-2 p-1 bg-light rounded-pill">
                  <li className="nav-item"><button className={`nav-link d-flex align-items-center gap-2 ${activeView === 'all' ? 'active' : ''}`} onClick={() => handleViewChange('all')}><FaTicketAlt /> Semua Tiket <span className="badge rounded-pill bg-secondary">{totalTickets}</span></button></li>
