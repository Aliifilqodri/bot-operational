...
function TicketCard({ ticket, picList, refreshData, onImageClick, onTextClick }) {
  ...
  const handleSetPic = async (e) => {
    ...
    try {
      await axios.patch(`${API_URL}/${ticket._id}/set-pic`, { pic: selectedPic });
  ...
  const handleReplyAndComplete = async () => {
    ...
    try {
      await axios.post(`${API_URL}/${ticket._id}/reply`, { balasan });
      await axios.patch(`${API_URL}/${ticket._id}/complete`);
      toast.success('Balasan terkirim & tiket selesai');
      setBalasan('');
      refreshData();
    } catch (err) {
      console.error(err);
      toast.error('Gagal mengirim balasan / menyelesaikan tiket');
    }
  };
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="text" value={balasan} onChange={(e) => setBalasan(e.target.value)} placeholder="Tulis balasan & selesaikan tiket..." className="form-control" />
